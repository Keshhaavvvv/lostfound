...
/*
    File: index.php
    Purpose: Main Dashboard (Dark Theme + Location & Category Filter).
*/
include 'db.php';
...
if (isset($_GET['location']) && !empty($_GET['location'])) {
    $location_filter = $conn->real_escape_string($_GET['location']);
    $search_sql .= " AND location = '$location_filter'";
}
...
// Fetch Items (Now includes Location & Category check)
...
<head>
    ...
    <style>
        ...
        .hero { 
            max-width: 900px; margin: 3rem auto; padding: 0 1.5rem; text-align: center; 
            position: relative;
        }
        ...
        /* --- NEW SEARCH BAR (With Filter) --- */
        .search-container {
            position: relative; max-width: 800px; margin: 0 auto 2.5rem auto;
        ...
        .search-group:focus-within { border-color: var(--primary); box-shadow: 0 0 0 4px rgba(99, 102, 241, 0.2); }

        /* Custom Select Styling */
        .filter-select {
            background: transparent; border: none; border-right: 1px solid var(--border);
            color: var(--text-main); font-size: 0.9rem; padding: 0 15px;
            outline: none; cursor: pointer; max-width: 150px;
            appearance: none; /* Hide default arrow to use custom background image if desired */
        }
        .filter-select option { background: var(--bg-card); color: white; }

        .search-input {
            width: 100%; padding: 18px 15px; border: none;
        ...
        .search-btn {
            ...
            width: 60px; cursor: pointer; font-size: 1.2rem;
        ...
        .card-meta { display: flex; gap: 10px; font-size: 0.8rem; color: #818cf8; margin-bottom: 0.8rem; flex-wrap: wrap; }
        .meta-item { display: flex; align-items: center; gap: 4px; background: rgba(99, 102, 241, 0.1); padding: 4px 8px; border-radius: 6px; }
        ...
        @media (max-width: 768px) { 
            ...
            .search-container { flex-direction: column; background: var(--bg-card); border-radius: 20px; border: 1px solid var(--border); padding: 10px; gap: 0; }
            .search-group { border: none; box-shadow: none; background: transparent; flex-direction: column; }
            .filter-select { width: 100%; max-width: 100%; border-right: none; border-bottom: 1px solid var(--border); padding: 10px; }
            .search-btn { width: 100%; border-radius: 10px; padding: 10px; margin-top: 10px; }
...
<body>
        ...
        <form action="" method="GET" class="search-container">
            <div class="search-group">
                <select name="location" class="filter-select">
                    ...
                    <option value="Campus General" <?php echo ($location_filter == 'Campus General') ? 'selected' : ''; ?>>General</option>
                    <option value="Library" <?php echo ($location_filter == 'Library') ? 'selected' : ''; ?>>Library</option>
                    <option value="Canteen" <?php echo ($location_filter == 'Canteen') ? 'selected' : ''; ?>>Canteen</option>
                    <option value="Computer Lab" <?php echo ($location_filter == 'Computer Lab') ? 'selected' : ''; ?>>Comp Lab</option>
                    <option value="Parking Lot" <?php echo ($location_filter == 'Parking Lot') ? 'selected' : ''; ?>>Parking</option>
                    <option value="Auditorium" <?php echo ($location_filter == 'Auditorium') ? 'selected' : ''; ?>>Auditorium</option>
                    <option value="Sports Ground" <?php echo ($location_filter == 'Sports Ground') ? 'selected' : ''; ?>>Sports Ground</option>
                </select>
                ...
                <select name="category" class="filter-select">
                    ...
                    <option value="Stationery" <?php echo ($category_filter == 'Stationery') ? 'selected' : ''; ?>>Stationery</option>
                    <option value="Clothing" <?php echo ($category_filter == 'Clothing') ? 'selected' : ''; ?>>Clothing</option>
                    <option value="Keys" <?php echo ($category_filter == 'Keys') ? 'selected' : ''; ?>>Keys</option>
                    <option value="Sports Gear" <?php echo ($category_filter == 'Sports Gear') ? 'selected' : ''; ?>>Sports Gear</option>
                    <option value="Other" <?php echo ($category_filter == 'Other') ? 'selected' : ''; ?>>Other</option>
                </select>
                
                <input type="text" name="search" class="search-input" placeholder="Search item..." value="<?php echo htmlspecialchars($search); ?>">
    ...
    <div class="container">
        <div id="grid-lost" class="item-grid active">
                ...
                <?php while($row = $result_lost->fetch_assoc()): ?>
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <div class="card-meta">
                                <span class="meta-item">📍 <?php echo htmlspecialchars($row['location'] ?? 'General'); ?></span>
                                <span class="meta-item"><?php echo getCategoryIcon($row['category']); ?> <?php echo htmlspecialchars($row['category'] ?? 'Other'); ?></span>
                            </div>
            ...
            <?php else: ?>
                <div style="grid-column: 1/-1; text-align: center; padding: 40px; color: #94a3b8;">
                    <?php echo ($search || $location_filter || $category_filter) ? "No lost items match your filters." : "No lost items reported recently."; ?>
                </div>
            <?php endif; ?>
        </div>
        ...
        <div id="grid-found" class="item-grid">
            ...
            <?php else: ?>
                <div style="grid-column: 1/-1; text-align: center; padding: 40px; color: #94a3b8;">
                    <?php echo ($search || $location_filter || $category_filter) ? "No found items match your filters." : "No found items reported recently."; ?>
                </div>
            <?php endif; ?>
        </div>
    </div>
